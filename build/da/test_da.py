...
def test_xarg_validation():
    ...
    check("valid passes", parse_da_xarg({"da": {"segs": [[0, 10], [10, 20]]}}) is not None)
    check("missing -> None", parse_da_xarg(None) is None)
    check("no da key -> None", parse_da_xarg({"other": 1}) is None)
    check("empty segs -> None", parse_da_xarg({"da": {"segs": []}}) is None)
    check("overlap -> None", parse_da_xarg({"da": {"segs": [[0, 10], [5, 20]]}}) is None)
    check("backwards -> None", parse_da_xarg({"da": {"segs": [[10, 5]]}}) is None)
    check("bad win -> None", parse_da_xarg({"da": {"segs": [[0, 5]], "win": 0}}) is None)
    check("sink>win -> None", parse_da_xarg({"da": {"segs": [[0, 5]], "win": 8, "sink": 16}}) is None)
    check("bool win -> None", parse_da_xarg({"da": {"segs": [[0, 5]], "win": True}}) is None)
    check("float ref -> None", parse_da_xarg({"da": {"segs": [[0, 5.5]]}}) is None)
    check("nested junk -> None", parse_da_xarg({"da": {"segs": ["x"]}}) is None)
    r = parse_da_xarg({"da": {"segs": [[10, 20], [0, 10]]}})
